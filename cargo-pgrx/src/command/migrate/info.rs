...
use super::version::{
    parse_version_lossy, resolve_prev_version, resolve_target_version,
};
...
pub fn build_migrate_plan(req: PlanRequest<'_>) -> eyre::Result<MigratePlan> {
    ...
    let out_dir = req
        ...
        .output_dir
        .map(|d| d.0)
        .unwrap_or(req.dirs.sql_dir.0);
    let output_file = out_dir.join(format!(
        "{}--{}--{}.sql",
        req.extname, prev_ver_str, target_ver_str
    ));
...
mod tests {
    ...
    fn build_migrate_plan_serializes_json_properly() {
        ...
        fs::write(
            unreleased_dir.join("5903.rename_test.sql"),
            "SELECT 1;",
        )
        .unwrap();
        fs::write(
            unreleased_dir.join("6221.unfielded.sql"),
            "-- depends-on: 5903\nSELECT 2;",
        )
        .unwrap();
