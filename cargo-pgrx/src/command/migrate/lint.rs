...
#[derive(
    Debug, Clone, Copy, PartialEq, Eq, clap::ValueEnum, Default, serde::Serialize, serde::Deserialize,
)]
...
pub fn catalog_unreleased_objects(
    fragments: &[MigrationFragment],
) -> BTreeMap<String, FragmentId> {
...
pub fn get_changed_fragment_files(
    ...
    let unreleased_canonical = unreleased_dir.canonicalize().unwrap_or_else(|_| unreleased_dir.to_path_buf());
...
pub fn run_lint_checks(
    ...
    let fragments_to_lint: Vec<&MigrationFragment> = if let Some(base_ref) = &config.base_ref {
        ...
        raw_fragments
            .iter()
            .filter(|f| changed_filenames.contains(&f.filename))
            .collect()
    } else {
...
pub fn render_lint_diagnostics(
...
impl CommandExecute for Check {
    ...
    fn execute(self) -> eyre::Result<()> {
        ...
        let config = LintConfig {
            deny_mixed_objects: self.deny_mixed_objects,
            base_ref: self.base,
        };
...
"#;
    ...
    fn catalogs_unreleased_objects_correctly() {
        let f1 = MigrationFragment {
            ...
            dependencies: vec![],
            sql_content: "CREATE TABLE t1 (id int); CREATE FUNCTION f1() RETURNS void AS $$ $$ LANGUAGE sql;".to_string(),
        };
    ...
    fn detects_mixed_objects_when_flag_enabled() {
        ...
        assert!(issues[0].message.contains("mixes modifications to already-released objects (TABLE released_table)"));
    }
}
