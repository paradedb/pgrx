...
pub fn assemble_release_upgrade_script(params: AssembleParams<'_>) -> eyre::Result<PathBuf> {
    ...
    let target_ver = parse_version_lossy(target_ver_str).ok_or_else(|| {
        eyre!("invalid semver for target version `{target_ver_str}`")
    })?;
    ...
    if params.control_update == ControlFileUpdate::Update {
        ...
        let control_path = if let Some(out) = params.dirs.output_dir && out.0.join(&control_filename).exists() {
            out.0.join(&control_filename)
        } else if let Some(crate_dir) = params.dirs.crate_dir && crate_dir.0.join(&control_filename).exists() {
            crate_dir.0.join(&control_filename)
        } else {
            return Err(eyre!(
                "could not find `{control_filename}` to update default_version in `{}` or `{}`",
                params.dirs.output_dir.map(|d| d.0.display().to_string()).unwrap_or_else(|| "<none>".to_string()),
                params.dirs.crate_dir.map(|d| d.0.display().to_string()).unwrap_or_else(|| "<none>".to_string())
            ));
        };

        crate::command::migrate::version::update_control_file_default_version(&control_path, target_ver_str)?;
...
impl CommandExecute for Assemble {
    ...
    fn execute(self) -> eyre::Result<()> {
        ...
        let control_update = if self.update_control {
            ControlFileUpdate::Update
        } else {
            ControlFileUpdate::Skip
        };
...
mod tests {
    ...
    fn assemble_errors_when_no_fragments_and_not_allowed() {
        ...
        assert!(res.is_err());
        assert!(res.unwrap_err().to_string().contains("no unreleased SQL migration fragments found"));
    }
